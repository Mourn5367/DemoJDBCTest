...
public interface MemberRepository03 extends JpaRepository<Member, Integer>
{
    ...
    public int insertMethod(@Param("e_name")String name , @Param("e_age")Integer age, @Param("e_email")String email);

    @Transactional
    @Modifying
    @Query(value = "update Member set name = :e_name, age = :e_age, email = :e_email where id = :e_id")
    public int updateMethod(@Param("e_name")String name , @Param("e_age")Integer age, @Param("e_email")String email, @Param("e_id")Integer id);


    @Transactional
    @Modifying
    @Query(value = "delete from Member where id = :e_id")
    public int deletetMethod(@Param("e_id") Integer e_id);
